...
@womens_stores_with_less_than_one_million = Store.where(womens_apparel: true).where("annual_revenue < 1000000")

@womens_stores_with_less_than_one_million.each do |store|
  p "Store Name: #{store.name}, Annual Revenue: #{store.annual_revenue}"
end
